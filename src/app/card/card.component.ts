...
export class CardComponent implements OnInit {
  ...
  @Input('persons') set _persons(value: any[]) {
    this.persons = value;
    this.personsRef = value;
  }
  constructor(private modalService: BsModalService, private personService: PersonsService) {
  ...
  editPerson(idx) {
    if (!this.imageSelected) {
      this.personService
        ...
        .subscribe(response => {
          let obj = {};
          obj['img_url'] = response["secure_url"];
          obj['name'] = this.persons[idx].name;
          obj['desc'] = this.persons[idx].desc;
          obj['_id'] = this.persons[idx]._id;

          this.personService.editPersonByID(obj).subscribe(res => {
            console.log(res);
      ...
      this.personService.editPersonByID(this.persons[idx]).subscribe(res => {
        console.log(res);
        window.location.reload();
      }, error => {
        console.log(error);
  ...
  deletePerson(idx) {
    this.personService.deletePersonByID(this.persons[idx]._id).subscribe(res => {
      console.log(res);
      window.location.reload();
    }, error => {
      console.log(error);
  ...
  updateUrl(e, idx) {
    this.persons[idx].img_url = 'https://material.angular.io/assets/img/examples/shiba1.jpg';
  }
